...
$$d(x)$$ is its coefficients in base 10, the fmap of $$Y\xrightarrow{i}X$$ is the function $$T(Y)\xrightarrow{T(i)}T(X)$$ defined by $$T(i)([(y,s)])=[(i(y),s)]$$ so $$T(i)(d(x))=[(10^p,r)]$$ and when we apply $$h$$ we obtain the familiar expression $$\sum_r*10^k$$ which is equal to $$x$$
